...
from collators.qwen2_5_omni_7b_rerank import Qwen2_5Omni7BDataCollatorRerank
from data.dataset_audioverse_rerank import AudioVerse_rerank
from loaders.qwen2_5_omni_7b_thinker import Qwen2_5Omni7BModelLoader
...
def train():
    ...
    # 方案2：使用自定义过滤器只过滤特定警告（推荐）
    root_logger = logging.getLogger("root")
    root_logger.addFilter(SystemPromptWarningFilter())
    ...
    # Set random seeds for reproducibility and proper distributed training
    ...
    loader = Qwen2_5Omni7BModelLoader(
    ...
    audio_folder = data_args.audio_folder[0]
    metadata_path = data_args.metadata_path[0]
    train_dataset = AudioVerse_rerank(
        audio_folder=audio_folder,
        metadata_path=metadata_path,
        tokenizer=tokenizer,
        # rerank_option=data_args.rerank_option,
    )
    ...
    # data collator
    data_collator = Qwen2_5Omni7BDataCollatorRerank(
        tokenizer=tokenizer,
        processor=processor,
    )
    ...
    t = trainer.get_train_dataloader()
